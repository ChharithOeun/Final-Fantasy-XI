"""Deep weapons — top-of-ladder underwater weapon forge.

The Mythic tier already exists in the canonical FFXI weapon
ladder; these are something separate. DEEP weapons are the
underwater-specific apex: forged in the wave temple, bound
to a name, never duplicated.

Each weapon is forged through 5 mandatory stages, each
opening a new trait slot. The whole chain costs months of
work and only one Deep weapon per player at a time.
...
--------------
    ForgeStage int enum
    DeepTrait dataclass (frozen)
    DeepWeapon dataclass (frozen)
    DeepWeapons
        ...
        .weapon_for(player_id) -> Optional[DeepWeapon]
        .traits_of(player_id) -> tuple[DeepTrait, ...]
...
class _ForgeState:
    ...
    weapon_kind: str
    started_at: int
    traits: list[DeepTrait] = dataclasses.field(default_factory=list)
...
class DeepWeapons:
    ...
    def start_forge(
        ...
        if player_id in self._forges:
            return False  # one Deep weapon per player
...
__all__ = [
    "ForgeStage", "DeepTrait", "DeepWeapon",
    "DeepWeapons",
]
